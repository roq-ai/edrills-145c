...
import { deleteScenarioById } from 'apiSdk/scenarios';
...
function OrganizationViewPage() {
  ...
  const scenarioHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('drill', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/drills/create?organization_id=${data?.id}`}>
              ...
              {hasAccess('scenario', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/scenarios/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.scenario?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/scenarios/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.name}</Td>
                            ...
                            <Td>
                              ...
                              {hasAccess('scenario', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    scenarioHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
