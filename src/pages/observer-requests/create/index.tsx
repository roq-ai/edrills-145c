...
import { createObserverRequest } from 'apiSdk/observer-requests';
...
function ObserverRequestCreatePage() {
  ...
  const handleSubmit = async (values: ObserverRequestInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createObserverRequest(values);
      resetForm();
